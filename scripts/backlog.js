#!/usr/bin/env node
/**
 * Backlog worklist — tag-based catalog of `backlog` zettels.
 *
 * Usage:
 *   node scripts/backlog.js             # ANSI table
 *   node scripts/backlog.js --markdown  # plain markdown
 */
...
const zettels = byTag(loadAll(), "backlog");
const edges = markdown ? [] : parseConnections();

if (markdown) {
  console.log(`# Backlog (${zettels.length} items)\n`);
  for (const z of zettels) {
    console.log(`- **${z.title}** \`${z.slug}\` [${z.tags.join(", ")}]`);
  }
} else {
  console.log(`\n${c.bold}BACKLOG${c.reset} — ${zettels.length} items\n`);
