...
**MIR lowering:** `src/lowering/leaf.ts` `foreign()` reads `ctx.declarations`; partial vs saturated behaviour matches primops (`docs/MIR-LOWERING.md` §FFI, `src/lowering/functions/materialize.ts`).
...
No `.ffi.c` / `.ffi.beam` pairing appears in `compile.ts`; only `.ffi.js` is referenced there.
