...
[[selective-cps]] (the rejected alternative documented under [[direct-style-lowering.adr]]) treats CPS as an inspiration drawn "broadly" from languages including Koka, while Yap's MIR path **favors direct style over global CPS** — `Shift`/`Reset` lower to explicit blocks and jumps (`src/lowering/`), not ubiquitous continuation arguments. Current lowering uses **direct-style MIR + closure conversion**.
