...
Verification focuses on liquid predicates; multiplicity checking is deferred in `src/verification/V2/` (see verification module layout). `VerificationArtefacts` carries Z3 VC + optional synthesized type (`src/verification/V2/types.ts`), not usage accumulation.
