...
**Verified in Yap:** `src/verification/V2/` translates refinements and λ-calculus fragments to Z3 (`z3-solver`), generates verification conditions, and runs the solver (tests under `src/verification/__tests__/`). Pipeline explorer exposes VC text via `EB.Mod.verify` + pretty printers (`src/cli/explore/pipeline.ts`).
