...
In **MIR lowering**, saturated foreign calls are accumulated then emitted as direct calls; partial applications become forwarding closures (`src/lowering/functions/materialize.ts` `reify` / `partial`).
