...
This file is a **registry**, not a constraint. It records tags and labels that have been used in this ZK.

- Tags are generated by the content of zettels, not assigned from a preapproved list.
- When a draft uses a tag not listed here, **add it** — do not substitute an existing tag. Swapping a specific tag for a broader one (e.g. `automation` → `tooling`) destroys query precision.
- Tags are **atomic facets**, not categories. Compound meaning comes from tag combinations + relationships, not compound tags.
- Same rule applies to connection labels in `connections.md`.
- Merge tags only when two are genuinely synonymous, not merely similar.
- Prefer specificity over generality when coining new tags.
...
### Thread Lifecycle Tags (apply to thread zettels)
